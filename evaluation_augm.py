...
import torchvision.transforms.functional as T2
...
for item in tqdm(metadata_teapots, desc="Processed Images", unit="Image"):
    # CONCEPTS
    image = Image.open(item['image_path'])
    sample = transform(image).unsqueeze(0).to(device)

    # AUGMENTATION
    #sample_augm = T2.rotate(sample, 180)
    #sample_augm = add_gaussian_noise(sample)
    sample_augm = T2.rotate(sample, 10)

    concepts_verystrong_augm, relevance_verystrong_augm = get_concepts_relevance_range(sample_augm, 0, 0.25, class_index=0)
    concepts_strong_augm, relevance_strong_augm = get_concepts_relevance_range(sample_augm, 0.25, 0.5, class_index=0)
    concepts_low_augm, relevance_low_noise = get_concepts_relevance_range(sample_augm, 0.5, 0.75, class_index=0)
    concepts_verylow_augm, relevance_verylow_augm = get_concepts_relevance_range(sample_augm, 0.75, 1, class_index=0)
    ...
    # CONCEPTS MOST SIMILAR IMAGE
    image_similar = Image.open(best_match["image_path"])
    sample_similar = transform(image_similar).unsqueeze(0).to(device)

    concepts_similar, relevance_similar = get_concepts_relevance_range(sample_similar, 0, 0.25, class_index=1)
    ...
    # UNIQUE CONCEPT FOR BOTH IMAGES
    c1_verystrong, c2_verystrong = generate_contrastive_explanation(concepts_verystrong_augm, concepts_similar)
    ...
    c1_strong, c2_strong = generate_contrastive_explanation(concepts_strong_augm, concepts_similar_strong)
    length_explanation_strong = len(c1_strong) + len(c2_strong)
    strong_relevance.append(length_explanation_strong)

    c1_low, c2_low = generate_contrastive_explanation(concepts_low_augm, concepts_similar_low)
    length_explanation_low = len(c1_low) + len(c2_low)
    low_relevance.append(length_explanation_low)

    c1_verylow, c2_verylow = generate_contrastive_explanation(concepts_verylow_augm, concepts_similar_verylow)
    length_explanation_verylow = len(c1_verylow) + len(c2_verylow)
    verylow_relevance.append(length_explanation_verylow)

# RESULTS IN JSON (CHANGE NAME OF THE FILE)
with open("verystrong_relevance_rot10.json", "w") as f:
    json.dump(verystrong_relevance, f, indent=2)

with open("strong_relevance_rot10.json", "w") as f:
    json.dump(strong_relevance, f, indent=2)

with open("low_relevance_rot10.json", "w") as f:
    json.dump(low_relevance, f, indent=2)

with open("verylow_relevance_rot10.json", "w") as f:
